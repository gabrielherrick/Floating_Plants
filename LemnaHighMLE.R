...
lemna.control <- filter(lemna, treatment == "H") #
##################################################
...
plot.exp.line <- plot.pop +
  ...
  annotate("text", x = 1.5, y = 265, label = "Exponential; r = 0.67", 
           color = "#E69F00", size=8)+
  annotate( "text", x=1.5, y=240, label= "Weight < 0.001%", 
            color="#E69F00", size=8)
plot.exp.line

png("Lemna_High_ExpLine.png")
# or tiff("plot.tiff")
print(plot.exp.line)
dev.off()
...
# 3.2 # MLE
fit.log <- mle2(LL.log, start = list(rate = 1.1, K = 320, mu = -6.4, sigma = 23))
# see results
summary(fit.log)
# Assign names to results
...
plot.log.line <- plot.pop +
  ...
  annotate( "text", x=1.5, y=265, label= "Logistic; r= 1.1, K = 321", 
            color="#56B4E9", size=8)+
  annotate( "text", x=1.5, y=240, label= "Weight = 100%", 
            color="#56B4E9", size=8)
plot.log.line

png("Lemna_High_LogLine.png")
# or tiff("plot.tiff")
print(plot.log.line)
dev.off()
