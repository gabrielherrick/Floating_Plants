...
lemna.control <- filter(lemna, treatment == "L") #
##################################################
...
plot.exp.line <- plot.pop +
  ...
  annotate("text", x = 1.5, y = 185, label = "Exponential; r = 0.58", 
           color = "#E69F00", size=8)+
  annotate( "text", x=1.5, y=160, label= "Weight = 3%", 
            color="#E69F00", size=8)
plot.exp.line

png("Lemna_Low_ExpLine.png")
# or tiff("plot.tiff")
print(plot.exp.line)
dev.off()
...
# 3.2 # MLE
fit.log <- mle2(LL.log, start = list(rate = .84, K = 242, mu = -2.4, sigma = 18))
# see results
summary(fit.log)
# Assign names to results
...
plot.log.line <- plot.pop +
  ...
  annotate( "text", x=1.5, y=185, label= "Logistic; r= 0.84, K = 243", 
            color="#56B4E9", size=8)+
  annotate( "text", x=1.5, y=160, label= "Weight = 97%", 
            color="#56B4E9", size=8)
plot.log.line

png("Lemna_Low_LogLine.png")
# or tiff("plot.tiff")
print(plot.log.line)
dev.off()
