...
fit.exp <- mle2(LL.exp, start = list(rate = .5, mu = 0, sigma = 20))
# See the results
summary(fit.exp)
...
plot.exp.line <- plot.pop +
  ...
  annotate("text", x = 1.5, y = 160, label = "Exponential; r = 0.47", 
           color = "#E69F00", size=8)+
  annotate( "text", x=1.5, y=140, label= "Weight = 99.75%", 
            color="#E69F00", size=8)
plot.exp.line

png("Salvinia_High_ExpLine.png")
# or tiff("plot.tiff")
print(plot.exp.line)
dev.off()
...
# 3.2 # MLE
fit.log <- mle2(LL.log, start = list(rate = .75, K = 200, mu = -10, sigma = 10))
# see results
summary(fit.log)
# Assign names to results
...
plot.log.line <- plot.pop +
  ...
  annotate( "text", x=2.5, y=160, label= "Logistic; r= 0.59, K = 281", 
            color="#56B4E9", size=8)+
  annotate( "text", x=2.5, y=140, label= "Weight = 0.0025%", 
            color="#56B4E9", size=8)
plot.log.line

png("Salvinia_High_LogLine.png")
# or tiff("plot.tiff")
print(plot.log.line)
dev.off()
