...
# 1.0 # Load data
# 1.1 # Get data
full.data <- read.csv( "SalviniaData.csv", header = T)
data <- tbl_df(full.data)
# remove any populations once they crash
data <- filter(data, !population == 0)

##################################################
# THIS CHANGES WITH TREATMENT LEVEL !!!!!        #
data <- filter(data, treatment == "C") #
##################################################
population <- data$population
time <- data$time 

# 1.3.0 # Plot raw data
plot.pop <- ggplot(data=data, aes(x=time , y=population))+
  geom_point(shape=1, size=3)+
  xlab("Week")+
  ylab("Salvinia population size")
plot.pop

# 1.3.1 # Save the plot to file
png("Salvinia_Control_nolines.png")
# or tiff("plot.tiff")
print(plot.pop)
dev.off()
...
fit.exp <- mle2(LL.exp, start = list(rate = .2, mu = 0, sigma = 2))
# See the results
summary(fit.exp)
...
plot.exp.line <- plot.pop +
  ...
  annotate("text", x = 1.5, y = 31, label = "Exponential; r = 0.16", 
           color = "#E69F00", size=8)+
  annotate( "text", x=1.5, y=29, label= "Weight = 75%", 
            color="#E69F00", size=8)
plot.exp.line

png("Salvinia_Control_ExpLine.png")
# or tiff("plot.tiff")
print(plot.exp.line)
dev.off()
...
# 3.2 # MLE
fit.log <- mle2(LL.log, start = list(rate = .3, K = 110, mu = -10, sigma = 5))
# see results
summary(fit.log)
# Assign names to results
...
plot.log.line <- plot.pop +
  ...
  annotate( "text", x=2.5, y=31, label= "Logistic; r= 0.19, K = 110", 
            color="#56B4E9", size=8)+
  annotate( "text", x=2.5, y=29, label= "Weight = 25%", 
            color="#56B4E9", size=8)
plot.log.line

png("Salvinia_Control_LogLine.png")
# or tiff("plot.tiff")
print(plot.log.line)
dev.off()
