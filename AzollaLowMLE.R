...
plot.exp.line <- plot.pop +
  ...
  annotate("text", x = 2.5, y = 78, label = "Exponential; r = 0.34", 
           color = "#E69F00", size=8)+
  annotate( "text", x=2.5, y=70, label= "Weight = 1%", 
            color="#E69F00", size=8)
plot.exp.line

png("Azolla_Low_ExpLine.png")
# or tiff("plot.tiff")
print(plot.exp.line)
dev.off()
...
# 3.2 # MLE
fit.log <- mle2(LL.log, start = list(rate = 1, K = 110, mu = 0, sigma = 10))
# see results
summary(fit.log)
# Assign names to results
...
plot.log.line <- plot.pop +
  ...
  annotate( "text", x=2.5, y=78, label= "Logistic; r= 0.73, K = 94", 
            color="#56B4E9", size=8)+
  annotate( "text", x=2.5, y=70, label= "Weight = 99%", 
            color="#56B4E9", size=8)
plot.log.line

png("Azolla_Low_LogLine.png")
# or tiff("plot.tiff")
print(plot.log.line)
dev.off()
