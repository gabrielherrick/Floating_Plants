...
# 1.0 # Load data
# 1.1 # Get data
full.data <- read.csv( "AzollaData.csv", header = T)
...
plot.pop <- ggplot(data=data, aes(x=time , y=population))+
  ...
  xlab("Week")+
  ylab("Azolla population size")
plot.pop

# 1.3.1 # Save the plot to file
png("Azolla_Control_nolines.png")
# or tiff("plot.tiff")
print(plot.pop)
dev.off()
...
fit.exp <- mle2(LL.exp, start = list(rate = .2, mu = 3, sigma = 10))
# See the results
summary(fit.exp)
...
plot.exp.line <- plot.pop +
  ...
  annotate("text", x = 2.5, y = 52, label = "Exponential; r = 0.23", 
           color = "#E69F00", size=8)+
  annotate( "text", x=2.5, y=48, label= "Weight = 11%", 
            color="#E69F00", size=8)
plot.exp.line

png("Azolla_Control_ExpLine.png")
# or tiff("plot.tiff")
print(plot.exp.line)
dev.off()
...
# 3.2 # MLE
fit.log <- mle2(LL.log, start = list(rate = .3, K = 50, mu = 10, sigma = 15))
# see results
summary(fit.log)
# Assign names to results
...
plot.log.line <- plot.pop +
  ...
  annotate( "text", x=2.5, y=52, label= "Logistic; r= 0.77, K = 50", 
            color="#56B4E9", size=8)+
  annotate( "text", x=2.5, y=48, label= "Weight = 89%", 
            color="#56B4E9", size=8)
plot.log.line

png("Azolla_Control_LogLine.png")
# or tiff("plot.tiff")
print(plot.log.line)
dev.off()
