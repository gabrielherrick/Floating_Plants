...
plot.exp.line <- plot.pop +
  ...
  annotate("text", x = 2.5, y = 160, label = "Exponential; r = 0.43", 
           color = "#E69F00", size=8)+
  annotate( "text", x=2.5, y=140, label= "Weight = 74%", 
            color="#E69F00", size=8)
plot.exp.line

png("Azolla_High_ExpLine.png")
# or tiff("plot.tiff")
print(plot.exp.line)
dev.off()
...
# 3.2 # MLE
fit.log <- mle2(LL.log, start = list(rate = .58, K = 222, mu = -1.1, sigma = 23))
# see results
summary(fit.log)
# Assign names to results
...
plot.log.line <- plot.pop +
  ...
  annotate( "text", x=2.5, y=160, label= "Logistic; r= 0.57, K = 227", 
            color="#56B4E9", size=8)+
  annotate( "text", x=2.5, y=140, label= "Weight = 26%", 
            color="#56B4E9", size=8)
plot.log.line

png("Azolla_High_LogLine.png")
# or tiff("plot.tiff")
print(plot.log.line)
dev.off()
